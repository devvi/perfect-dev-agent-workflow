...
/**
 * Check collision between player and boss body segments
 * Returns true if player head overlaps any boss segment
 */
export function checkBossCollision(playerHead, boss) {
  if (!boss || !playerHead) return false;
  const allSegments = [...(boss.segments1 || []), ...(boss.segments2 || [])];
  return allSegments.some(seg => seg.x === playerHead.x && seg.y === playerHead.y);
}

/**
 * Check collision with boss room pillars
 * playerPillar: player head on BOSS_PILLAR cell → damage
 * Returns 'player' | 'boss' | null
 */
export function checkPillarCollision(entity, room) {
  if (!entity || !room || !room.tiles) return null;
  for (const pos of [{ cx: 3, cy: 3 }, { cx: 15, cy: 3 }, { cx: 3, cy: 15 }, { cx: 15, cy: 15 }]) {
    if (room.tiles[pos.cy] && room.tiles[pos.cy][pos.cx] === 'B') {
      // Check if entity is at this position
      if (Array.isArray(entity)) {
        // Snake array — check head position
        if (entity[0] && entity[0].x === pos.cx && entity[0].y === pos.cy) return 'player';
      } else if (entity.x !== undefined) {
        if (entity.x === pos.cx && entity.y === pos.cy) return 'player';
      }
    }
  }
  return null;
...
/**
 * Check food decay state — returns true if food should be flashing
 */
export function checkFoodDecay(food) {
  if (!food || food.decayTimer === undefined) return false;
  return food.decayTimer <= 80;
}
