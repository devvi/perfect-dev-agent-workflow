...
/**
 * Apply damage to a boss column — removes tail segment from specified column.
 * If column becomes empty, reduces boss HP by 1.
 */
export function applyBossDamage(boss, column) {
  const seg = boss[column];
  if (!seg) return;
  if (seg.length > 0) {
    seg.pop();
  }
  // If column is now empty (either just emptied or was already empty),
  // boss takes 1 damage
  if (seg.length === 0) {
    boss.hp -= 1;
    boss.colHp -= 1;
...
/**
 * Handle boss death — marks boss as defeated and transitions to defeated phase
 */
...
/**
 * Spawn food with physics bounce properties at given position
 * Adds a food item with decay timer and bounce velocity to state.foods
 */
export function spawnFoodWithPhysics(x, y, state) {
  const food = {
    x,
    y,
    decayTimer: 120,
...
/**
 * Update food decay timers — decrement timers, update flash rates,
 * remove expired food
 */
export function updateFoodDecay(state) {
  if (!state.foods) return state;
  const remaining = [];
  for (const food of state.foods) {
    food.decayTimer -= 1;
    if (food.decayTimer <= 0) continue;
    // Calculate flash rate: faster as timer approaches 0
    if (food.decayTimer <= 80) {
      food.flashRate = Math.max(1, Math.floor(food.decayTimer / 10));
    }
    remaining.push(food);
  }
