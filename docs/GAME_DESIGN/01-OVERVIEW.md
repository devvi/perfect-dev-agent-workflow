...
玩家出生时：
- **蛇身长度：2 节**（头部 + 一个身体节）——降低初始容错，迫使玩家尽快移动/进食
- **起始房间（START）：** 放置 2 个普通敌人，防止玩家原地挂机安全发育
- **起始房间食物：** 仍为 3 个（用于恢复长度的途径保留）

| 属性 | 值 | 设计意图 |
|------|-----|---------|
| 初始蛇身 | 2 节 | 一次受伤即进入危险状态，创造紧迫感 |
| 起始房间敌人 | 2 个 | 迫使玩家立刻观察环境并做出移动决策 |
| 起始房间食物 | 3 个 | 吃食物可恢复长度到 3+，提供应对危险的手段 |

**设计决策：** 初始蛇身从 3 节改为 2 节，配合起始房间加入敌人，使游戏开头从"安全等待"变为"立即行动"。
