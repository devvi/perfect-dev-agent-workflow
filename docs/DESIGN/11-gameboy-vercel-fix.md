# Design: #11 — Fix GameBoy 404 on Vercel Deployment
...
**Why This Wasn't Caught:** The original design doc (`docs/DESIGN/5-gameboy-snake-game.md`) correctly separated the engine into `src/` for testability, but did not account for the Vercel deployment constraint. The `vercel.json` has no build configuration, rewrites, or redirects to bridge this gap.
...
### Selected Approach: Option A — Move engine into `public/src/`
...
|------|-------------------|------------|
| `src/gameboy-snake-engine.js` | Move to `public/src/gameboy-snake-engine.js` | 1 file move |
| `public/gameboy.html` | Change import from `'../src/...'` to `'./src/...'` | ±1 |
| `tests/gameboy-snake.test.js` | Change import from `'../src/...'` to `'../public/src/...'` | ±1 |
| `tests/gameboy-vercel.test.js` | NEW: deployment verification tests | +50 |
| `docs/DESIGN/5-gameboy-snake-game.md` | Update to note `public/src/` path | ±2 |

---

## 4. Verification Checklist

- [ ] `GET /src/gameboy-snake-engine.js` returns HTTP 200 (not 404) — verify in Browser DevTools Network tab on deployed URL
...
- [ ] File existence: `public/src/gameboy-snake-engine.js` exists and exports match original API
- [ ] No regression — `about.html` still loads correctly
